...
private:
    ...
    std::vector<int> find_accounts_by_name(std::string name)
    {
        ...
        for (auto i = 0; i < accounts.size(); i++)
        {
            ...
            Customer *c = accounts[i]->get_customer();
            //get name of customer
            if (c->get_name() == name)
            {
                //get number of account and push back if true
                int n = accounts[i]->get_account_number();
                user_accounts.push_back(n);
            }
        }
    ...
    Customer *find_customer(std::string name)
    {
        ...
        {
            //if found  "in the system" return the customer
